...
function getTotalBookings() {
    ...
    // Log the POST data
    file_put_contents(__DIR__ . '/../logs/debug.log', date('Y-m-d H:i:s') . " - getTotalBookings called\n", FILE_APPEND);
    ...
    try {
        ...
        try {
            ...
            file_put_contents(__DIR__ . '/../logs/debug.log', date('Y-m-d H:i:s') . " - Attempting to fetch bookings with limit=$limit, offset=$offset\n", FILE_APPEND);
            ...
            // Log any exceptions
            file_put_contents(__DIR__ . '/../logs/debug.log', date('Y-m-d H:i:s') . " - Exception fetching bookings: " . $e->getMessage() . "\n", FILE_APPEND);
            $bookings = [];
        }
        
        // Log the bookings data
        if (is_array($bookings)) {
            file_put_contents(__DIR__ . '/../logs/debug.log', date('Y-m-d H:i:s') . " - Bookings count: " . count($bookings) . "\n", FILE_APPEND);
        } else {
            file_put_contents(__DIR__ . '/../logs/debug.log', date('Y-m-d H:i:s') . " - Bookings is not an array: " . print_r($bookings, true) . "\n", FILE_APPEND);
